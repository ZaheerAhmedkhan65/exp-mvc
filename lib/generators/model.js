...
const DependencyChecker = require("../helpers/dependency-checker");

function createModel(name, fields = [], projectRoot) {
    const checker = new DependencyChecker(projectRoot);

    // Check if mongoose is installed when creating a model
    if (!checker.isInstalled('mongoose')) {
        console.log('📦 Installing mongoose for model support...');
        checker.installPackage('mongoose');
    }
